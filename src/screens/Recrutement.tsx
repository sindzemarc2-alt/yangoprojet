        ...
        <ScrollView contentContainerStyle={styles.scroll}>
          ...
          {step === 1 && (
            <View>
              ...
              <TextInput style={styles.input} placeholder="Nom et Prénoms" onChangeText={(t) => setFormData({...formData, nom: t})} />
              <TextInput style={styles.input} placeholder="Téléphone (ex: 6XXXXXXXX)" keyboardType="phone-pad" onChangeText={(t) => setFormData({...formData, phone: t})} />
              <TextInput style={styles.input} placeholder="Adresse Email" keyboardType="email-address" onChangeText={(t) => setFormData({...formData, email: t})} />
            </View>
          )}
          ...
          {step === 2 && (
            <View>
              ...
              <TextInput style={styles.input} placeholder="Années d'expérience de conduite (Permis B)" keyboardType="numeric" onChangeText={(t) => setFormData({...formData, experience: t})} />
              <TextInput style={styles.input} placeholder="Avez-vous déjà travaillé dans le transport ? (Oui/Non)" onChangeText={(t) => setFormData({...formData, ancien: t})} />
            </View>
          )}
          ...
          <View style={styles.footer}>
            {step > 1 && <TouchableOpacity onPress={() => setStep(step - 1)} style={styles.backBtn}><Text>Retour</Text></TouchableOpacity>}
            <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  scroll: { padding: 20 },
  headerTitle: { fontSize: 32, fontWeight: '900', color: '#000', marginTop: 20 },
  stepIndicator: { fontSize: 16, color: '#333', marginBottom: 30, fontWeight: '600' },
  label: { fontSize: 22, fontWeight: '800', marginBottom: 20, color: '#000' },
  input: { backgroundColor: '#FFF', padding: 18, borderRadius: 12, marginBottom: 15, borderWidth: 1, borderColor: '#000' },
  ...
  backBtn: { padding: 20, backgroundColor: '#DDD', borderRadius: 12 },
  nextBtn: { padding: 20, backgroundColor: '#000', borderRadius: 12, alignItems: 'center' },
  btnText: { color: '#FFD700', fontWeight: 'bold', fontSize: 16 },
